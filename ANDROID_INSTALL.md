# Claudex on Android (Termux)

Run Claudex on Android using Termux + proot Ubuntu.
...
Claudex requires [Bun](https://bun.sh) to build. Bun doesn't support Android natively, but works inside a proot Ubuntu environment running inside Termux.
...
```
✓ Built Claudex v0.1.7 → dist/cli.mjs
```
...
The Claudex banner and provider info box will appear. If you see a login screen asking for Anthropic, your env vars aren't set — check Step 7 and make sure you ran `source ~/.bashrc`.
...
Both fail due to Claudex's large system prompt (~50K tokens):
